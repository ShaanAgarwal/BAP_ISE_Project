...
fig_timeline = px.line(
    publication_counts_all_years,
    x="Year",
    y="Publication Count",
    title="Timeline of Paper Publications by Year",
)
...
fig_department_distribution = px.bar(
    department_distribution,
    x="Department",
    y="Paper Count",
    title="Department-wise Distribution of Papers",
)
...
app.layout = html.Div(
    ...
    children=[
        html.H1(
            children="BAP ISE DASHBOARD",
            style={
                "text-align": "center",
                "font-weight": "bold",
                "font-family": "Arial, sans-serif",
                "color": "#F0F0F0",  # Light pastel shade
                "margin-bottom": "10px",  # Add spacing below the title
            },
        ),
        html.Div(
            children="Research Paper Publications S.P.I.T",
            style={
                "text-align": "center",
                "font-size": "20px",
                "font-family": "Arial, sans-serif",
                "color": "#FFDAB9",  # Light pastel shade
                "margin-bottom": "30px",  # Add spacing below the subheading
            },
        ),
        html.Div(
            [
                dcc.Dropdown(
                    ...
                    options=[
                        ...
                        for paper in df["Type of Paper"].fillna("Unknown").unique()
                    ],
            ...
            style={
                "background-color": "#FFDAB9",  # Light pastel shade
                "padding": "20px",
                "margin-bottom": "20px",
                "border-radius": "10px",
            },
            ...
            [
                html.Div(
                    [
                        dcc.Graph(
                            figure=fig_timeline,
                            style={"background-color": "#FFE4B5"},  # Light pastel shade
                        )
                    ],
                ...
                html.Div(
                    [
                        dcc.Graph(
                            figure=fig_department_distribution,
                            style={"background-color": "#FFDAB9"},  # Light pastel shade
                        )
                    ],
                    style={"width": "50%", "display": "inline-block"},
                ),
            ],
            style={"margin-bottom": "30px"},  # Add spacing between sections
        ),
        dcc.Dropdown(
                ...
                html.Div(
                    [
                        html.Div(
                            ...
                            style={
                                "background-color": "#FFE4B5",  # Light pastel shade
                                "padding": "20px",
                                "margin-bottom": "20px",
                                "border-radius": "10px",
                            },
        ...
        html.Div(
            [
                ...
                html.Div(
                    [
                        html.H3(
                            "Paper Count by Application Department and Type of Publication",
                            style={
                                "text-align": "center",
                                "font-weight": "bold",
                                "font-size": "24px",
                        ...
                        dcc.Graph(id="paper-department-count"),
                    ],
                    style={
                        "background-color": "#FFE4B5",  # Light pastel shade
                        "padding": "20px",
        ...
        html.Div(
            [
                ...
                dcc.Graph(id="top-guides-professors-bar-chart"),
            ],
            style={
                "background-color": "#FFDAB9",  # Light pastel shade
                "padding": "20px",
        ...
        html.Div(
            [
                html.H1(
                    "Year Wise Publication By Guides",
                    style={
                        "text-align": "center",
                        "font-weight": "bold",
                        "font-size": "28px",
...
@app.callback(Output("pie-chart", "figure"), [Input("paper-dropdown", "value")])
def update_pie_chart(selected_paper):
    ...
    fig = px.pie(
        publication_counts,
        values="Count",
        names="Type of Publication",
        title=f"Distribution of Publication Types for {selected_paper}",
    )
    return fig


@app.callback(
    Output("total-count-output", "children"), [Input("paper-dropdown", "value")]
)
...
def update_charts(selected_application_type):
    ...
    filtered_df.loc[:, "Application Course"] = filtered_df["Application Course"].fillna(
        "Unknown"
    )
    filtered_df.loc[:, "Application Department"] = filtered_df[
        "Application Department"
    ].fillna("Unknown")
    filtered_df.loc[:, "Application Class"] = filtered_df["Application Class"].fillna(
        "Unknown"
    )
    ...
    fig_bar_updated = px.bar(
        counts,
        x="Year",
        y="Count",
        title=f"Count of Each Year for {selected_application_type}",
    )
    fig_sunburst_updated = px.sunburst(
        filtered_df,
        path=["Application Course", "Application Department", "Application Class"],
    )
...
@app.callback(Output("publication-graph", "figure"), [Input("guide-dropdown", "value")])
def update_graph(selected_guide):
    filtered_df = df[
        (df["Guid 1 Name"] == selected_guide) | (df["Guid 2 Name"] == selected_guide)
    ]
    ...
    publication_counts = (
        publication_counts.set_index("Year").reindex(all_years).fillna(0).reset_index()
    )
    hover_text = []
    customdata = []
    for year in publication_counts["Year"]:
        papers = filtered_df[filtered_df["Start Year"] == year][
            "Title of Paper"
        ].tolist()
        hover_text.append(
            f"Year: {year}<br>Publication Count: {publication_counts[publication_counts['Year'] == year]['Publication Count'].values[0]}"
        )
    ...
    fig = px.line(
        ...
        x="Year",
        y="Publication Count",
        title=f"Publications Over Time for {selected_guide}",
        hover_data={
            "Year": False,
    ...
    fig.update_traces(
        hovertemplate="<b>Year</b>: %{x}<br><b>Publication Count</b>: %{y}<br><br>%{customdata}"
    )
...
def update_paper_count_graphs(selected_guide):
    ...
    fig_paper_type_count = px.bar(
        paper_type_counts,
        x="Type of Paper",
        y="Count",
        title="Paper Count by Type of Paper",
    )
    publication_department_counts = (
        filtered_df.groupby(["Type of Publication", "Application Department"])
        .size()
        .reset_index(name="Count")
    )
    fig_paper_department_count = px.bar(
        publication_department_counts,
        x="Type of Publication",
        y="Count",
        color="Application Department",
        barmode="group",
        title="Paper Count by Type of Publication and Application Department",
    )
...
def generate_wordcloud(selected_application_type):
    ...
    titles_no_stopwords = " ".join(
        [word for word in titles.split() if word.lower() not in stop_words]
    )
    wordcloud = WordCloud(
        stopwords=stop_words, width=800, height=400, background_color="white"
    ).generate(titles_no_stopwords)
...
@app.callback(
    Output("sankey-diagram", "figure"), [Input("application-type-dropdown", "value")]
)
def update_sankey_diagram(selected_application_type):
    filtered_df = df[df["Application Type"] == selected_application_type]
    department_type_counts = (
        filtered_df.groupby(["Application Department", "Type of Publication"])
        .size()
        .reset_index(name="Count")
    )
        ...
        data=[
            go.Sankey(
                ...
                link=dict(
                    ...
                    target=[
                        targets.tolist().index(target) + len(sources)
    ...
    fig_sankey.update_layout(
        title_text="Flow of Publications by Department and Type", font=dict(size=10)
    )
    return fig_sankey
...
@app.callback(
    ...
    [Input("top-guides-input", "value"), Input("domain-input", "value")],
)
def update_top_guides_professors_bar_chart(top_count, keyword):
    ...
    if keyword:
        ...
        top_entities = (
            filtered_df[["Guid 1 Name", "Guid 2 Name"]]
            .stack()
            .value_counts()
            .nlargest(top_count)
        )
        ...
        top_entities = (
            pd.concat([df["Guid 1 Name"], df["Guid 2 Name"]])
            .value_counts()
            .nlargest(top_count)
        )
...
@app.callback(Output("plot", "figure"), [Input("year-dropdown", "value")])
def update_plot(selected_year):
    ...
    guide_titles_df = pd.DataFrame(
        index=all_guides, columns=["Title of Paper", "Paper Count"]
    )
    for guide in all_guides:
        titles = filtered_df[
            (filtered_df["Guid 1 Name"] == guide)
            | (filtered_df["Guid 2 Name"] == guide)
        ]["Title of Paper"].tolist()
    ...
    for guide, titles in guide_titles_df.iterrows():
        fig.add_trace(
            go.Bar(
                x=[guide],
                y=[titles["Paper Count"]],
                name=guide,
                hovertext=f"Paper Count: {titles['Paper Count']}<br>{titles['Title of Paper']}",
                hoverinfo="text",
            )
        )
    fig.update_layout(
...
@app.callback(
    Output("sunburst-chart2", "figure"), [Input("application-type-dropdown", "value")]
)
def update_sunburst_chart(selected_application_type):
    ...
    fig_sunburst_updated2 = px.sunburst(
        filtered_df, path=["Type of Paper", "Type of Publication", "Conference_Type"]
    )
